...
import com.brightbankdemo.weavrfundstransfer.model.SendBulkCreateRequest;
import com.brightbankdemo.weavrfundstransfer.model.SendCreateRequest;
...
public interface SendsService {
   ...
   * Cancels send transactions that are scheduled to be executed in the future.
    * @param body  (optional)
   */
  @RequestLine("POST /sends/bulk/cancel")
  @Headers({
    "Content-Type: application/json",
    "Accept: ",  })
  void sendCancel(TransactionBulkCancelRequest body);
  ...
  void sendCreate(SendCreateRequest body, @Param("idempotency_ref") String idempotency_ref);

  /**
   * Create a bulk of send transactions
   * Deprecated in favour of the [Bulk operation](Operations/#operation/bulksSendCreate).  Sends funds between multiple managed accounts and managed cards belonging to *different* corporate or consumer identities.  The Send Profile (configured in the Multi Portal) specified determines the behaviour and restrictions of the transaction (for example, fees). 
    * @param body  (optional)
    * @param idempotency_ref A unique call reference generated by the caller that, taking into consideration the payload as well as the operation itself, helps avoid duplicate operations. Idempotency reference uniqueness is maintained for at least 24 hours. (optional)
   */
  @RequestLine("POST /sends/bulk/create")
  @Headers({
    "Content-Type: application/json",
    "Accept: ",
    "idempotency-ref: {idempotency_ref}"  })
  void sendCreateBulk(SendBulkCreateRequest body, @Param("idempotency_ref") String idempotency_ref);
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAChallenge)  Starts the verification process for a send in which a one-time password is sent to a device belonging to the logged-in user that was previously enrolled through the &#x60;/authentication_factors/otp/{channel}&#x60; endpoint.  You should only start this process if the send &#x60;state&#x60; is &#x60;PENDING_CHALLENGE&#x60;.  _Note that on the Sandbox Environment, text messages are not sent and the one-time-password is always \\\&quot;123456\\\&quot;._ 
    * @param id The unique identifier of the send transaction. (required)
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAChallengePush)  Starts the verification process for a Send in which a push notification is sent to a device belonging to the logged-in user that was previously enrolled through the &#x60;/authentication_factors/push/{channel}&#x60; endpoint.  You should only start this process if the Send transaction &#x60;state&#x60; is &#x60;PENDING_CHALLENGE&#x60;. 
    * @param id The unique identifier of the send transaction. (required)
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAVerify)  Completes the verification process for a Send.  If the outcome of the verification is successful, the send is executed.  If not verified challenge expires after 5 minutes and the number of incorrect OTP attempts is limited to reduce the risk of fraud.  _Note that on the Sandbox Environment, text messages are not sent and the &#x60;verificationCode&#x60; is always \\\&quot;123456\\\&quot;._ 
    * @param id The unique identifier of the send transaction. (required)
    * @param channel The unique identifier for the channel. (required)
   */
   ...
   * Retrieves all the send transactions performed by the logged-in identity.
    ...
    * @param executedFrom Filter for send transactions executed after &#x60;executedFrom&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all Send transactions. (optional)
    * @param executedTo Filter for send transactions executed before &#x60;executedTo&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all Send transactions. (optional)
   ...
   */
  @RequestLine("GET /sends?offset={offset}&limit={limit}&profileId={profileId}&instrumentId={instrumentId}&source.id={source_id}&source.type={source_type}&state={state}&createdFrom={createdFrom}&createdTo={createdTo}&executedFrom={executedFrom}&executedTo={executedTo}&tag={tag}")
  @Headers({
    "Accept: ",  })
  void sendsGet(@Param("offset") Integer offset, @Param("limit") Integer limit, @Param("profileId") Object profileId, @Param("instrumentId") Object instrumentId, @Param("source_id") Object source_id, @Param("source_type") Object source_type, @Param("state") List state, @Param("createdFrom") Long createdFrom, @Param("createdTo") Long createdTo, @Param("executedFrom") Long executedFrom, @Param("executedTo") Long executedTo, @Param("tag") String tag);
     ...
     *   <li>executedFrom - Filter for send transactions executed after &#x60;executedFrom&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all Send transactions. (optional)</li>
     *   <li>executedTo - Filter for send transactions executed before &#x60;executedTo&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all Send transactions. (optional)</li>
