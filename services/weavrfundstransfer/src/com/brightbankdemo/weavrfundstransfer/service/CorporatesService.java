...
import com.brightbankdemo.weavrfundstransfer.model.InlineResponse200;
import com.brightbankdemo.weavrfundstransfer.model.InlineResponse409;
...
public interface CorporatesService {
  ...
  @Headers({
    ...
    "Accept: ",
    "idempotency-ref: {idempotency_ref}"  })
  void corporateCreate(@Param("idempotency_ref") String idempotency_ref);
  ...
  @Headers({
    ...
    "Accept: ",
    "idempotency-ref: {idempotency_ref}"  })
  void corporateRootUserEmailVerificationCodeSend(@Param("idempotency_ref") String idempotency_ref);
  ...
  @Headers({
    "Content-Type: application/json",
    "Accept: ",  })
  void corporateRootUserEmailVerify();

  /**
   * Update a corporate
   * Update the details of the logged-in corporate identity.  The following Corporate details are verified during due diligence (KYB) and cannot be updated via the API once the Corporate has been verified:   - Root User Name   - Root User Surname   - Root User Email   - Root User Mobile Country Code   - Root User Mobile Number   - Business Address   - Date of birth  *If these details need to be changed after due diligence is completed, please contact Customer Support to restart the Due Diligence process.* 
    * @param idempotency_ref A unique call reference generated by the caller that, taking into consideration the payload as well as the operation itself, helps avoid duplicate operations. Idempotency reference uniqueness is maintained for at least 24 hours. (optional)
   */
  ...
  @Headers({
    ...
    "Accept: ",
    "idempotency-ref: {idempotency_ref}"  })
  void corporateUpdate(@Param("idempotency_ref") String idempotency_ref);
