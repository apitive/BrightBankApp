...
public interface TransfersService {
   ...
   * Cancels transfers that are scheduled to be executed in the future.
    * @param body  (optional)
   */
  @RequestLine("POST /transfers/bulk/cancel")
  @Headers({
    "Content-Type: application/json",
    "Accept: ",  })
  void transferCancel(TransactionBulkCancelRequest body);
   ...
   * Retrieves all the transfer transactions performed by the logged-in identity.
    ...
    * @param executedFrom Filter for transfer transactions executed after &#x60;executedFrom&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all transfers. (optional)
    * @param executedTo Filter for transfer transactions executed before &#x60;executedTo&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all transfers. (optional)
   ...
   */
  @RequestLine("GET /transfers?offset={offset}&limit={limit}&profileId={profileId}&instrumentId={instrumentId}&state={state}&createdFrom={createdFrom}&createdTo={createdTo}&executedFrom={executedFrom}&executedTo={executedTo}&tag={tag}")
  @Headers({
    "Accept: ",  })
  void transfersGet(@Param("offset") Integer offset, @Param("limit") Integer limit, @Param("profileId") Object profileId, @Param("instrumentId") Object instrumentId, @Param("state") List state, @Param("createdFrom") Long createdFrom, @Param("createdTo") Long createdTo, @Param("executedFrom") Long executedFrom, @Param("executedTo") Long executedTo, @Param("tag") String tag);
     ...
     *   <li>executedFrom - Filter for transfer transactions executed after &#x60;executedFrom&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all transfers. (optional)</li>
     *   <li>executedTo - Filter for transfer transactions executed before &#x60;executedTo&#x60; timestamp. Timestamp is expressed in Epoch timestamp using millisecond precision. Leave out to fetch all transfers. (optional)</li>
