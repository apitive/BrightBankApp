...
import com.brightbankdemo.weavrfundstransfer.model.OutgoingWireTransferBulkCreateRequest;
import com.brightbankdemo.weavrfundstransfer.model.OutgoingWireTransferCreateRequest;
import com.brightbankdemo.weavrfundstransfer.model.TransactionBulkCancelRequest;
...
public interface OutgoingWireTransfersService {
   ...
   * Cancels outgoing wire transfers that are scheduled to be executed in the future.
    * @param body  (optional)
   */
  @RequestLine("POST /outgoing_wire_transfers/bulk/cancel")
  @Headers({
    "Content-Type: application/json",
    "Accept: ",  })
  void outgoingWireTransferCancel(TransactionBulkCancelRequest body);
  ...
  void outgoingWireTransferCreate(OutgoingWireTransferCreateRequest body, @Param("idempotency_ref") String idempotency_ref);

  /**
   * Create a bulk of outgoing wire transfer transactions
   * Deprecated in favour of the [Bulk operation](Operations/#operation/bulksOutgoingWireTransferCreate).  Transfers funds from multiple managed accounts to multiple external bank accounts.  The Outgoing Wire Transfer Profile (configured in the Multi Portal) specified determines the behaviour and restrictions of the transaction (for example, fees).  Note that even if the transaction is created successfully, you still need to check its state in order to determine if it is pending completion of an authentication challenge by the user.  
    * @param body  (optional)
    * @param idempotency_ref A unique call reference generated by the caller that, taking into consideration the payload as well as the operation itself, helps avoid duplicate operations. Idempotency reference uniqueness is maintained for at least 24 hours. (optional)
   */
  @RequestLine("POST /outgoing_wire_transfers/bulk/create")
  @Headers({
    "Content-Type: application/json",
    "Accept: ",
    "idempotency-ref: {idempotency_ref}"  })
  void outgoingWireTransferCreateBulk(OutgoingWireTransferBulkCreateRequest body, @Param("idempotency_ref") String idempotency_ref);
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAChallenge)  Starts the verification process for an outgoing wire transfer in which a one-time password is sent to a device belonging to the logged-in user that was previously enrolled through the &#x60;/authentication_factors/otp/{channel}&#x60; endpoint.  You should only start this process if the outgoing wire transfer &#x60;state&#x60; is &#x60;PENDING_CHALLENGE&#x60;.  _Note that on the Sandbox Environment, text messages are not sent and the one-time-password is always \\\&quot;123456\\\&quot;._ 
    * @param id The unique identifier of the Outgoing Wire Transfer transaction. (required)
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAChallengePush)  Starts the verification process for an outgoing wire transfer in which a push notification is sent to a device belonging to the logged-in user that was previously enrolled through the &#x60;/authentication_factors/push/{channel}&#x60; endpoint.  You should only start this process if the outgoing wire transfer &#x60;state&#x60; is &#x60;PENDING_CHALLENGE&#x60;. 
    * @param id The unique identifier of the Outgoing Wire Transfer transaction. (required)
   ...
   * Deprecated in favour of [Challenges endpoints](Challenges/#operation/multipleSCAVerify)  Completes the verification process for an outgoing wire transfer.  If the outcome of the verification is successful, the outgoing wire transfer transaction is executed.  If not verified challenge expires after 5 minutes and the number of incorrect OTP attempts is limited to reduce the risk of fraud.  _Note that on the Sandbox Environment, text messages are not sent and the &#x60;verificationCode&#x60; is always \\\&quot;123456\\\&quot;._ 
    * @param id The unique identifier of the Outgoing Wire Transfer transaction. (required)
    * @param channel The unique identifier for the channel. (required)
   */
   ...
   * Retrieves all the outgoing wire transfer transactions performed by the logged-in identity.
    ...
    * @param executedFrom Filter for outgoing wire transfer transactions executed after &#x60;executedFrom&#x60; timestamp, expressed in Epoch timestamp using millisecond precision. Leave out to fetch all outgoing wire transfer transactions. (optional)
    * @param executedTo Filter for Outgoing Wire Transfer transactions executed before &#x60;executedTo&#x60; timestamp, expressed in Epoch timestamp using millisecond precision. Leave out to fetch all outgoing wire transfer transactions. (optional)
   ...
   */
  @RequestLine("GET /outgoing_wire_transfers?offset={offset}&limit={limit}&profileId={profileId}&instrument={instrument}&sourceInstrument.id={sourceInstrument_id}&sourceInstrument.type={sourceInstrument_type}&state={state}&createdFrom={createdFrom}&createdTo={createdTo}&executedFrom={executedFrom}&executedTo={executedTo}&tag={tag}")
  @Headers({
    "Accept: ",  })
  void outgoingWireTransfersGet(@Param("offset") Integer offset, @Param("limit") Integer limit, @Param("profileId") Object profileId, @Param("instrument") Object instrument, @Param("sourceInstrument_id") Object sourceInstrument_id, @Param("sourceInstrument_type") Object sourceInstrument_type, @Param("state") List state, @Param("createdFrom") Long createdFrom, @Param("createdTo") Long createdTo, @Param("executedFrom") Long executedFrom, @Param("executedTo") Long executedTo, @Param("tag") String tag);
     ...
     *   <li>executedFrom - Filter for outgoing wire transfer transactions executed after &#x60;executedFrom&#x60; timestamp, expressed in Epoch timestamp using millisecond precision. Leave out to fetch all outgoing wire transfer transactions. (optional)</li>
     *   <li>executedTo - Filter for Outgoing Wire Transfer transactions executed before &#x60;executedTo&#x60; timestamp, expressed in Epoch timestamp using millisecond precision. Leave out to fetch all outgoing wire transfer transactions. (optional)</li>
